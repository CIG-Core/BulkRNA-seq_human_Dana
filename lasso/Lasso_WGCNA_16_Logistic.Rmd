...
library(openxlsx)
output_path <- "/ix/cigcore/proj/biswas/code/Sissi/BulkRNA-seq_human_Dana/lasso/TPM/Logistic/posModule(152gene)_TPM_16.xlsx"
write.xlsx(gene_TPM_treat, file = output_path)
```
```{r}
library(xfun)
xfun::embed_files(c('./TPM/Logistic/posModule(152gene)_TPM_16.xlsx'))
```
...
# response_variable <- as.factor(coldata$reverse_treatment)
  alpha <- 1
...
plot.roc(
  ...
  col = "royalblue",
  legacy.axes = TRUE,
  main = paste("Nested-CV Lasso model (3 genes) in negative module (63 genes)\n Area under the curve (AUC):", round(auc, digits = 2))
)
...
output_path <- "/ix/cigcore/proj/biswas/code/Sissi/BulkRNA-seq_human_Dana/lasso/TPM/Logistic/negModule(63gene)_TPM_16.xlsx"
write.xlsx(gene_TPM_treat, file = output_path)
```

```{r}
library(xfun)
xfun::embed_files(c('./TPM/Logistic/negModule(63gene)_TPM_16.xlsx'))
```
...
plot.roc(
  ...
  col = "royalblue",
  legacy.axes = TRUE,
  main = paste("Nested-CV Lasso model (2 genes) in negative module (303 genes)\n Area under the curve (AUC):", round(auc, digits = 2))
)
```
...
output_path <- "/ix/cigcore/proj/biswas/code/Sissi/BulkRNA-seq_human_Dana/lasso/TPM/Logistic/negModule(303gene)_TPM_16.xlsx"
write.xlsx(gene_TPM_treat, file = output_path)
```
```{r}
library(xfun)
xfun::embed_files(c('./TPM/Logistic/negModule(303gene)_TPM_16.xlsx'))
```
...
plot.roc(
  ...
  grid.lwd = 2,
  col = "royalblue",
  main = paste("Nested-CV Final model (11 genes)\n Area under the curve (AUC):", round(auc, digits = 2))
)
